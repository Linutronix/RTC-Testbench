...
sub main
{
	...
	while ($line = <STDIN>) {
		...
		if ($source eq "ptp4l") {
			next
				unless (($rms_ns, $max_ns, $freq_ppb, $delay_ns) = $line =~
				/rms\s+(-?\d+)\s+max\s+(-?\d+).*?freq\s+(-?\d+).*?delay\s+(-?\d+)/);
		} else {
			next
				unless (($offset_ns, $freq_ppb, $delay_ns) = $line =~
				/offset\s+(-?\d+)\s+\w+\s+freq\s+([+-]?\d+)\s+delay\s+(\d+)/);
		}
