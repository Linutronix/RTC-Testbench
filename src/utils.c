...
int prepare_frame_for_tx(const struct prepare_frame_config *frame_config)
{
	...
	if ((frame_config->frame_type == GENERICL2_FRAME_TYPE) &&
		...
		__le16 ecat_length = htole16(((frame_config->frame_length - 14) & 0x7FF) | 0x1000);
		__le16 cmd_length = htole16((frame_config->frame_length - 28) & 0x7FF);

		memcpy(&frame_config->frame_data[ECAT_TX_TIMESTAMP_OFF],
		       &(frame_config->tx_timestamp), sizeof(uint64_t));
		memcpy(&frame_config->frame_data[ECAT_LENGTH_OFF], &ecat_length, sizeof(__le16));
		memcpy(&frame_config->frame_data[ECAT_CMD_LENGTH_OFF], &cmd_length, sizeof(__le16));
